...
AI_CACHE_VERSION = "preboom_stock_brief_v1"
...
# ================= 暴涨前形态参数 =================
# 只基于你当前缓存里的 open / close 做判断，不改接口
PREBOOM_LOOKBACK = 60
PREBOOM_MIN_DAYS = 45
...
# 离平台高点不能太远，也不能已经突破太多
MIN_CLOSE_TO_BASE_HIGH = -4.0
MAX_BREAKOUT_ABOVE_BASE = 13.0

# 离20日均线太远，说明可能已经过热
MAX_CLOSE_ABOVE_MA20 = 22.0
...
def save_cache(cache_df):
    ...
    cache_df = cache_df.groupby("symbol", group_keys=False).tail(100)
...
def screen_from_cache(cache_df):
    ...
    识别目标：
    1. 前面有一段平台/缓慢抬升，不是已经连续暴涨。
    2. 5/10/20日均线开始转多。
    3. 股价贴近或小幅突破平台高点。
    4. 最近3/5/10日开始有动量，但不允许涨幅过热。
    5. 最近10日不能出现太多 7% 以上大涨，避免筛到已经爆发后的票。

    注意：
    你当前缓存只有 open / close，没有 high / low / volume。
    所以这里用收盘价近似判断平台突破，不能判断真实成交量放大。
    """
    print("🧮 正在从本地缓存中执行【暴涨前平台启动形态】筛选...")
    ...
    def range_score(value):
        ...
        if value < MIN_BASE_RANGE:
            return clip_score(value, 0, MIN_BASE_RANGE)
        elif value <= 28:
            return 100.0
        elif value <= MAX_BASE_RANGE:
            return clip_score(value, MAX_BASE_RANGE, 28, reverse=True)
        else:
            return 0.0
    ...
    for symbol, group in cache_df.groupby("symbol"):
        ...
        r30 = pct_change(close, 30)
        r60 = pct_change(close, min(59, len(close) - 1))
        ...
        latest_day_pct = float(last_10_daily.iloc[-1]) if len(last_10_daily.dropna()) > 0 else 0.0

        # 红K数量，保留这个字段，兼容原来的展示结构
        ...
        # 平台区间：取最近 35 天，但跳过最近 5 天，避免刚启动的几天污染平台高点
        ...
        if base is None or base.empty or len(base) < 20:
            continue
        ...
        ma_turning = latest_close >= ma5 and ma5 >= ma10 and ma10 >= ma20 * 0.98
        ma20_up = ma20 >= ma30 * 0.98
        long_trend_ok = latest_close >= ma60 * 0.92
        ...
        # 最近开始有启动迹象，但还没有暴涨
        early_accel = (
            (r3 >= 1.5 and r3 <= 12.0) or
            (r5 >= 3.0 and r5 <= MAX_R5_GAIN) or
            (r10 >= 5.0 and r10 <= MAX_R10_GAIN)
        )
        ...
        not_overheated = (
            ...
            big_up_count_10 <= 1 and
            limit_like_count_20 <= 1
        )
        ...
        # 至少有一点阳线推动
        red_ok = red_count_5 >= 3 or red_count_7 >= 4 or red_count_10 >= 5

        # 硬条件
        if not base_ok:
            continue

        if not near_base_high:
            continue

        if not not_too_far_breakout:
            continue
        ...
        # 突破位置分：贴近平台高点，或者刚刚小幅突破最好
        if close_to_base_high < -4:
            breakout_score = 0.0
        elif close_to_base_high <= 3:
            breakout_score = clip_score(close_to_base_high, -4, 3)
        elif close_to_base_high <= 8:
            breakout_score = 100.0
        elif close_to_base_high <= MAX_BREAKOUT_ABOVE_BASE:
            breakout_score = clip_score(close_to_base_high, MAX_BREAKOUT_ABOVE_BASE, 8, reverse=True)
        else:
            breakout_score = 0.0

        # 动量分：要有动，但不能太猛
        accel_score = (
            clip_score(r3, 0, 9) * 0.35 +
            clip_score(r5, 2, 16) * 0.35 +
            clip_score(r10, 4, 28) * 0.30
        )

        # 均线粘合分：启动前通常 MA5/10/20 不会离太远
        compact_score = clip_score(ma_spread, 9, 1.5, reverse=True)
        ...
        if r10 > 28:
            risk_penalty += clip_score(r10, 28, MAX_R10_GAIN) * 0.15

        if r20 > 45:
            risk_penalty += clip_score(r20, 45, MAX_R20_GAIN) * 0.18

        if close_above_ma20 > 16:
            risk_penalty += clip_score(close_above_ma20, 16, MAX_CLOSE_ABOVE_MA20) * 0.15

        if latest_day_pct >= BIG_UP_PCT:
            risk_penalty += 8
        ...
        if not risk_notes:
            risk_notes.append("形态仍偏早期，但需观察能否放量突破")
        ...
        red_days_detail.append(f"3日/5日/10日涨幅：{r3:.2f}%/{r5:.2f}%/{r10:.2f}%")
...
**为什么像爆发前形态：**
用1-2条 bullet 分析可能原因，比如平台整理、均线转多、贴近平台高点、短线刚启动、资金试盘等。
...
def write_blog_post(stock_list):
    ...
    md_content = f"""---
title: "🚀 【暴涨前形态雷达】平台蓄势 / 均线转多 / 贴近突破股票扫描 ({today_date})"
date: {post_time}
categories:
    - 量化研报
tags:
